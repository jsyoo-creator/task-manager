...
function CompareTable() {
  const rows: [string, string, string][] = [
    ...
    ['화면', '텍스트만 오가는 검은 창', '채팅창 + 파일 미리보기 + 변경 비교 화면이 한 창에'],
    ['변경 승인', '터미널에 y/n으로 답하거나 설정으로 자동 승인', '파일별로 "수락/거절" 버튼 클릭 (변경 전/후 비교 제공)'],
    ['여러 작업 동시 진행', '터미널 창을 여러 개 띄워야 함', '사이드바에서 작업 여러 개를 나란히 관리'],
    ['추천 대상', '터미널이 익숙하거나 자동화를 많이 쓸 사람', '터미널이 낯설고, 눈으로 확인하며 진행하고 싶은 사람'],
  ];
  return (
...
// "이럴 때 이렇게 말하세요" 예시 카드
function PromptCard({ situation, example, tip }: { situation: string; example: string; tip?: React.ReactNode }) {
  return (
    <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 10, padding: '16px 18px', marginBottom: 12 }}>
      <div style={{ fontSize: 13.5, fontWeight: 700, color: '#4338ca', marginBottom: 8 }}>{situation}</div>
      <div style={{ fontSize: 14, color: '#1e293b', background: '#f8fafc', borderRadius: 8, padding: '10px 14px', lineHeight: 1.6 }}>
        "{example}"
      </div>
      {tip && <div style={{ fontSize: 12.5, color: '#64748b', marginTop: 8 }}>{tip}</div>}
    </div>
  );
}
...
function WordsTable() {
  const rows: [string, string][] = [
    ['"이상해요"', '"OO가 나와야 하는데 OO가 나와요" 처럼 지금 상태와 원하는 상태를 둘 다 말하기'],
    ['"여기요", "이 부분"', '화면 이름 + 항목 이름을 콕 집기 (예: "설정 > 엑셀 관리 탭의 복지몰 파트")'],
    ['"중앙/가운데로"', '"글자 자체가 칸의 가로·세로 정가운데로" 처럼 결과물을 그림 그리듯 설명'],
    ['"위/아래로 옮겨줘"', '"3번째 줄로 옮겨줘"처럼 몇 번째인지 숫자로'],
    ['"빠르게/편하게"', '"누르면 1초 안에 떠야 해"처럼 구체적인 기준으로'],
    ['"이쁘게 해줘"', '스크린샷을 첨부하거나 "OO 화면이랑 비슷한 느낌으로"'],
    ['"다 고쳐줘"', '어떤 화면·어떤 기능인지 하나씩 짚어서 (한 번에 여러 개면 화면별로 나눠 말하기)'],
    ['"예전처럼"', '정확히 언제 어떤 모습이었는지 (스크린샷이 있으면 가장 확실함)'],
  ];
  return (
    <div style={{ overflowX: 'auto', margin: '14px 0 20px' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13.5 }}>
        <thead>
          <tr style={{ background: '#f1f5f9' }}>
            <th style={{ textAlign: 'left', padding: '9px 12px', fontWeight: 700, color: '#475569', border: '1px solid #e2e8f0', width: '30%' }}>애매해서 오해가 잘 생기는 말</th>
            <th style={{ textAlign: 'left', padding: '9px 12px', fontWeight: 700, color: '#475569', border: '1px solid #e2e8f0' }}>이렇게 바꿔 말하면 훨씬 정확해요</th>
          </tr>
        </thead>
        <tbody>
          {rows.map(([a, b]) => (
            <tr key={a}>
              <td style={{ padding: '9px 12px', border: '1px solid #e2e8f0', fontWeight: 600, color: '#b91c1c' }}>{a}</td>
              <td style={{ padding: '9px 12px', border: '1px solid #e2e8f0', color: '#047857' }}>{b}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
// UI 용어 사전 그룹 (용어 - 뜻/예시)
function GlossaryGroup({ title, items }: { title: string; items: [string, string][] }) {
  return (
    <div style={{ marginBottom: 20 }}>
      <div style={{ fontSize: 13.5, fontWeight: 700, color: '#1e293b', marginBottom: 8 }}>{title}</div>
      <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 10, overflow: 'hidden' }}>
        {items.map(([term, desc], i) => (
          <div key={term} style={{ display: 'flex', gap: 14, padding: '10px 16px', borderTop: i === 0 ? 'none' : '1px solid #f1f5f9' }}>
            <div style={{ flexShrink: 0, width: 130, fontWeight: 700, fontSize: 13.5, color: '#4338ca' }}>{term}</div>
            <div style={{ fontSize: 13.5, color: '#334155', lineHeight: 1.6 }}>{desc}</div>
          </div>
        ))}
      </div>
    </div>
...
function MethodTabs({ method, onChange }: { method: Method; onChange: (m: Method) => void }) {
  const tabs: { id: Method; label: string }[] = [
    { id: 'app', label: '데스크탑 앱으로 진행' },
    { id: 'cli', label: '터미널로 진행' },
  ];
  return (
          ...
          return (
            <li key={id}>
              <a
                href={`#${id}`}
                style={{
                  display: 'block', fontSize: 12.5, padding: '7px 10px', borderRadius: 6, textDecoration: 'none',
      ...
      <div style={{ maxWidth: 1040, margin: '0 auto', padding: '48px 24px 120px' }}>
        ...
        <div style={{ display: 'flex', gap: 40, alignItems: 'flex-start' }}>
          ...
          <section>
            ...
            <P>클로드에게 코드를 직접 만들고 고쳐달라고 요청하는 프로그램이 "Claude Code"입니다. 위 탭에서 원하는 방식을 선택하면 아래 안내가 그에 맞춰 바뀝니다.</P>
            <CompareTable />
          ...
          <section>
            ...
            <Callout tone="tip" title="이 순서가 왜 먼저인가요">
              GitHub 저장소를 미리 연결해두면, 클로드가 코드를 수정할 때마다 <IC>git push</IC>만으로 바로 배포까지 이어집니다(8번 참고).
              이 프로젝트도 <IC>jsyoo-creator/task-manager</IC> Private 저장소로 관리되고 있고, main 브랜치에 push하면 자동 배포됩니다.
            </Callout>
          </section>

          <section>
            <H2 id="examples">3. 무엇을 만들고 싶을 때 — 상황별 요청 예시</H2>
            <P>말주변이 없어도 괜찮습니다. 원하는 상황과 가장 비슷한 문장을 그대로 가져다 쓰고, 화면 이름·항목 이름만 바꾸면 됩니다.</P>

            <PromptCard
              situation="① 완전히 새로운 화면을 만들고 싶을 때"
              example="휴가 신청 화면을 새로 만들어줘. 날짜랑 사유를 입력받고, 등록하면 목록에 '대기' 상태로 나오게 해줘"
            />
            <PromptCard
              situation="② 목록에서 원하는 것만 골라보고 싶을 때 (검색/필터)"
              example="업무 목록에서 담당자 이름으로 검색할 수 있는 검색창을 추가해줘"
            />
            <PromptCard
              situation="③ 자동으로 계산되게 하고 싶을 때"
              example="세부 업무 시간을 다 더한 합계가 목록 맨 위에 자동으로 보이게 해줘"
            />
            <PromptCard
              situation="④ 특정 사람만 보게/누르게 하고 싶을 때 (권한)"
              example="삭제 버튼은 관리자 역할인 사람한테만 보이게 해줘"
            />
            <PromptCard
              situation="⑤ 무언가 생기면 알려주고 싶을 때 (알림)"
              example="새 업무가 등록되면 담당자한테 알림이 뜨게 해줘"
            />
            <PromptCard
              situation="⑥ 엑셀/파일로 뽑고 싶을 때"
              example="지금 보이는 목록을 엑셀 파일로 다운로드할 수 있는 버튼을 추가해줘"
            />
            <PromptCard
              situation="⑦ 디자인(색·모양)을 바꾸고 싶을 때"
              example="등록 버튼 색을 지금 회색 말고 보라색 계열로 바꿔줘"
            />
            <PromptCard
              situation="⑧ 기존 기능이 잘못됐을 때 (버그 신고)"
              example="업무관리 화면에서 날짜를 오늘로 바꾸면 목록이 사라지면 안 되는데 사라져 버려. 그대로 남아있어야 해"
              tip="버그 신고는 '어느 화면 → 무엇을 눌렀더니 → 어떻게 잘못 나오는지 → 원래 어때야 하는지' 4단계로 말하면 가장 정확합니다."
            />

            <Callout tone="tip" title="만능 공식 하나만 기억하기">
              <b>화면 이름</b> + <b>하고 싶은 동작</b> + <b>조건(누가/언제/어떻게)</b>. 이 세 가지만 채워서 문장을 만들면 대부분의 요청이 오해 없이 전달됩니다.
              예: "[업무관리 화면]에서, [상태를 '완료'로 바꾸면], [담당자한테만] 알림이 가게 해줘".
            </Callout>
          </section>

          <section>
            <H2 id="words">4. 클로드가 더 잘 알아듣는 표현으로 말하기</H2>
            <P>클로드는 화면을 보는 눈이 없습니다. 오직 <b>사용자가 입력한 글자(그리고 첨부한 스크린샷)</b>만으로 상황을 판단합니다.
            전화로 처음 보는 인테리어 업자에게 "저기 좀 이쁘게 해주세요"라고만 말하면 오해가 생기는 것과 같은 이치입니다. 아래 표처럼 바꿔 말하면 훨씬 정확하게 전달됩니다.</P>
            <WordsTable />
            <Callout tone="tip" title="클로드가 되묻거나 선택지를 주면, 정확하게 답하기">
              애매한 요청에는 클로드가 먼저 "어떤 화면 말씀이신가요?"처럼 되묻거나 몇 가지 선택지를 보여줄 수 있습니다. 이때 대충 고르지 말고 실제로 원하는 것과 정확히 일치하는지 확인하세요.
            </Callout>
          </section>
          ...
          <section>
            <H2 id="debug">7. 버그를 찾는 법 (실전 디버깅 노하우)</H2>
            <P>기능이 이상하게 동작할 때, 클로드는 아래와 같은 순서로 원인을 좁혀갑니다. 사용자가 이 흐름을 알고 협조하면 훨씬 빨리 해결됩니다.</P>

            <H3>7-1. "재현 경로"를 있는 그대로 믿고 그 경로부터 추적한다</H3>
            <P>사용자가 "이렇게 하니까 이런 문제가 생겨"라고 설명하면, 그 설명을 캐시나 오래된 데이터 탓으로 넘겨짚지 않고 <b>사용자가 말한 정확한 순서 그대로</b> 코드를 따라가며 원인을 찾습니다.</P>

            <H3>7-2. 정상 동작하는 "비슷한 다른 화면"과 코드를 비교한다</H3>
            <P>같은 종류의 기능이 화면 A에서는 되고 화면 B에서는 안 되면, A가 참조하는 코드 패턴을 검색해서 B와 정확히 뭐가 다른지 비교합니다.</P>
            <Callout tone="tip" title="실제 사례 (최근 있었던 일)">
            ...
            <H3>7-3. 데이터베이스를 직접 볼 수 없을 때 — 임시 진단 로그를 활용한다</H3>
            ...
            <P>실제로 위 7-2의 엑셀 관리 버그를 고친 뒤, "전체 팀 중 몇 개 파트가 실제로 영향을 받았는지" 확인할 때 이 방법을 써서 17개 파트 중 8개 파트가 영향받았다는 걸 바로 확인할 수 있었습니다.</P>

            <H3>7-4. 화면/레이아웃 버그는 "무엇이 어떻게 잘못됐는지" 스크린샷으로 먼저 재확인한다</H3>
            <P>색상이 이상하다, 정렬이 안 맞는다 같은 신고는 사람마다 표현이 달라 오해가 쉽습니다(사례 1, 3 참고). 코드를 고치기 전에 스크린샷으로 "정확히 무엇이 잘못됐는지"부터 짚고 시작하면 시행착오가 크게 줄어듭니다.</P>

            <H3>7-5. "코드 버그"인지 "데이터 자체의 문제"인지 구분한다</H3>
          ...
          <section>
            <H2 id="deploy">8. 서버/배포 세팅 (Vercel + Firebase)</H2>
            <P>"서버 세팅"이라고 하면 복잡하게 느껴지지만, 이 프로젝트는 직접 서버를 관리하지 않고 아래 두 서비스에 맡기는 구조입니다.</P>

            <H3>8-1. Vercel — 웹사이트를 실제로 띄워주는 곳</H3>
            <ol style={{ paddingLeft: 20, marginBottom: 16, color: '#334155' }}>
            ...
            <H3>8-2. Firebase — 데이터베이스 + 로그인 기능</H3>
            <ol style={{ paddingLeft: 20, marginBottom: 16, color: '#334155' }}>
            ...
            <H3>8-3. 두 서비스 연결하기 — 환경변수 등록</H3>
            <P>8-2에서 복사한 값 6개를 Vercel 프로젝트의 <b>Settings → Environment Variables</b>에 아래 이름으로 등록합니다(이 프로젝트에서 실제로 쓰는 이름).</P>
...
VITE_FIREBASE_APP_ID`}</Code>
          ...
          <section>
            <H2 id="cycle">9. 실전 사이클 — 기능 하나 만들어보기</H2>
          ...
          <section>
            <H2 id="pitfalls">10. 자주 하는 실수 &amp; 팁</H2>
            <ul style={{ paddingLeft: 20, marginBottom: 16, color: '#334155' }}>
          ...
          <section>
            <H2 id="checklist">11. 요약 체크리스트</H2>
            <ul style={{ paddingLeft: 20, marginBottom: 8, color: '#334155' }}>
              ...
              <li>☐ 요청할 때 "화면 이름 + 하고 싶은 동작 + 조건"으로 + 애매하면 스크린샷 첨부</li>
